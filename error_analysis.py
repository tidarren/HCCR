...
NUM_CLASSES = 879
TRAINING_SRC = 'create_selectNew_879' 

TEST_DIR = 'selectNew_{}'.format(NUM_CLASSES)
MODEL_PATH = 'model_{}.hdf5'.format(TRAINING_SRC)
...
def main():
    ...
    # train_data_path = os.path.join(BASE,'training_data',TRAINING_SRC)
    test_data_path = os.path.join(BASE,'training_data',TEST_DIR)
    Train_Data_Genetor = ImageDataGenerator( rescale = 1/255)
    Val_Generator = Train_Data_Genetor.flow_from_directory( test_data_path ,
                                                    target_size = (96,96),
    ...
    # with open('label2char_4843.pkl'.format(), 'wb') as f:
    ...
    with open('label2char_{}.pkl'.format(NUM_CLASSES), 'rb') as f:
    ...
    for pred, label, filename in zip(result, Val_Generator.labels, Val_Generator.filenames):
        ...
        elif TRAINING_SRC in ['create_selectNew_801']:
            log = (filename, prob, pred, label, answer_is, is_prob_low)
        elif TRAINING_SRC in ['create_selectNew_879', 'opendata_commonlyUsed_879', 'data_4843']:
            pred = 'isnull' if pred not in tranining_words_800 else pred
            label = 'isnull' if label not in tranining_words_800 else label
            answer_is = 'correct' if pred==label else 'wrong'
            log = (filename, prob, pred, label, answer_is, is_prob_low)
        logs.append(log)
    ...
    elif TRAINING_SRC in ['create_selectNew_801', 'create_selectNew_879', 'opendata_commonlyUsed_879', 'data_4843']:
        columns = ['filename','prob','pred','label','answer_is','is_prob_low']
