...
def preprocess_input(img, show_img=False):
	...
	# norm_img = normalize_bitmap(np.array(img, dtype=np.uint8))
	
	# prepr_img = np.array(preprocess_bitmap(norm_img), dtype=np.uint8).reshape(1, *IMG_SHAPE, 1)
	prepr_img = np.array(preprocess_bitmap(norm_img), dtype=np.uint8).reshape(*IMG_SHAPE, 1)
	if show_img:
		...
		plt.show()
	# print('out img.shape',prepr_img.shape)
	return prepr_img
